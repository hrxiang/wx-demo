...
<script>
	...
	export default {
		...
		methods: {
						...
						.boundingClientRect(data => {
							...
							if (this.inAli) {
								...
								e.changedTouches.unshift({
									x: (e.detail.clientX ?? e.touches[0]?.clientX) - data.left,
									y: (e.detail.clientY ?? e.touches[0]?.clientY) - data.top
								});
							} else {
								// 新增touches取值逻辑
								e.changedTouches.unshift({
									x: (e.detail.x ?? e.touches[0]?.pageX) - data.left,
									y: (e.detail.y ?? e.touches[0]?.pageY) - data.top - this.pageScrollTop
								});
							}
							if (move) {
					...
					if (move) {
						...
						this.emitMsg({
							...
							params: {
								type: "getIndex",
								event: {
									// x: e.detail.x, 修改为下面的值，修复环形图tooltip存在位置未从触摸点显示的问题
									x: e.detail.x - e.currentTarget.offsetLeft,
									y: e.detail.y - e.currentTarget.offsetTop
								},
								currentIndex: currentIndex,
			...
			_touchMoveGetIndex(e) {
				// 移动显示气泡
				const cid = this.cid;
				const currentIndex = cfu.instance[cid].getCurrentDataIndex(e);
				const legendIndex = cfu.instance[cid].getLegendDataIndex(e);
				this.emitMsg({
					name: 'getIndexWhileMoving',
					params: {
						type: "getIndexWhileMoving",
						event: {
