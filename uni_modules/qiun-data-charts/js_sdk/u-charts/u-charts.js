...
function splitPoints(points, eachSeries) {
	var delPoints = eachSeries.delActivePointIndex;
	var newPoints = [];
	var items = [];
...
function getYAxisTextList(series, opts, config, stack, yData) {
	// 在点击图例隐藏series数据后，因y轴值变化引起x刻度值位置移动
	if (yData.keepYAxisValue) { // 点击图例保持series不变
		series = opts.series;
	}
	...
	// sorted在获取最大值和最小值之前已经移除了所有null，如果只存在一条小于0的数，那么最大值和最小值都会是这个负数
	// 修复数组里只存在一条数据且小于0的情况
	...
	const yAxisList = generateYAxisTicks(maxData, minData, opts.yAxis.splitNumber);
	return yAxisList.reverse();
}

function generateYAxisTicks(maxRange, minRange, splitNumber, autoRound = true) {
	// 调整范围确保包含0（如果需要）
	let dataMin = Math.min(0, minRange);
	let dataMax = Math.max(0, maxRange);

	// 自动计算舍入精度
	let roundToNearest = 0;
	if (autoRound) {
		roundToNearest = calculateAutoRoundValue(dataMax, dataMin);
	}

	// 如果有舍入精度，对数据进行舍入处理
	if (roundToNearest > 0) {
		// 对最大值和最小值进行舍入处理，创建间隙
		dataMax = Math.ceil(dataMax / roundToNearest) * roundToNearest;
		dataMin = Math.floor(dataMin / roundToNearest) * roundToNearest;

		// 确保舍入后仍然包含原始数据范围
		if (dataMin > minRange) {
			dataMin -= roundToNearest;
		}
		if (dataMax < maxRange) {
			dataMax += roundToNearest;
		}
	}

	const range = dataMax - dataMin;
	const useInteger = range >= splitNumber;

	if (useInteger) {
		// 整数刻度 - 找到合适的整数步长
		let step = Math.ceil(range / splitNumber);

		// 计算最小刻度
		let minTick = Math.floor(dataMin / step) * step;

		// 计算最大刻度
		let maxTick = minTick + step * splitNumber;

		// 确保包含所有数据点
		while (maxTick < dataMax) {
			step += 1;
			minTick = Math.floor(dataMin / step) * step;
			maxTick = minTick + step * splitNumber;
		}
		...
		if (minRange <= 0 && maxRange <= 0) {
			maxTick = 0;
			// 重新计算最小刻度
			minTick = maxTick - step * splitNumber;
		}
		...
		for (let i = 0; i <= splitNumber; i++) {
			ticks.push(minTick + i * step);
		}

		return ticks;
	} else {
		// 小数刻度 - 找到一个"漂亮"的步长
		const rawStep = range / splitNumber;

		// 找到一个漂亮的步长 (0.1, 0.2, 0.5, 1, 2, 5等)
		let niceStep = getNiceStep(rawStep);

		// 计算最小刻度
		let minTick = Math.floor(dataMin / niceStep) * niceStep;

		// 计算最大刻度
		let maxTick = minTick + niceStep * splitNumber;

		// 确保包含所有数据点
		while (maxTick < dataMax) {
			// 找到下一个更漂亮的步长
			niceStep = getNextNiceStep(niceStep);
			minTick = Math.floor(dataMin / niceStep) * niceStep;
			maxTick = minTick + niceStep * splitNumber;
		}
		...
		if (minRange <= 0 && maxRange <= 0) {
			maxTick = 0;
			// 重新计算最小刻度
			minTick = maxTick - niceStep * splitNumber;
		}
		...
		const ticks = [];
		for (let i = 0; i <= splitNumber; i++) {
			ticks.push(parseFloat((minTick + i * niceStep).toFixed(2)));
		}
...
// 自动计算舍入精度
function calculateAutoRoundValue(maxValue, minValue) {
	// 计算数据的数量级
...
function getNiceStep(rawStep) {
	const niceSteps = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 2, 5, 10, 20, 50, 100];
	...
	// 如果没有找到合适的步长，返回原始步长向上取整
	return Math.ceil(rawStep);
}

// 找到下一个更漂亮的步长
function getNextNiceStep(currentStep) {
	const niceSteps = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 2, 5, 10, 20, 50, 100];

	for (let i = 0; i < niceSteps.length; i++) {
		if (niceSteps[i] > currentStep) {
			return niceSteps[i];
		}
	}
...
function drawPointShape(points, color, shape, context, opts) {
	...
	//新增逻辑，只绘制指定某个点出的shape
	const customIndex = opts.extra.line?.activePointIndex;
	const customDraw = customIndex !== null && customIndex !== undefined && customIndex !== -1;
	...
	if (opts.dataPointShapeType == 'hollow') {
		...
		context.setStrokeStyle(customDraw ? color : "#ffffff");
		context.setFillStyle(color);
		context.setLineWidth(2 * opts.pix);
	}
	if (shape === 'diamond') {
		points.forEach(function(item, index) {
			if (item !== null && (customDraw && customIndex === index || !customDraw)) {
...
function drawActivePoint(points, color, shape, context, opts, option, seriesIndex) {
	...
	if (shape === 'diamond') {
		...
		// 自定义中心位置 阶梯图选中横线中心，非x轴刻度位置
		// 范围值选择两刻度中心位置
		const customX = option.type === "step" && option.step === "middle";
		const spacing = opts.chartData.eachSpacing / 2; //两刻度点中心位置
		...
		points.forEach(function(item, index) {
			...
			if (item !== null && pointIndex == index) {
				...
				context.moveTo(x + 2.5 * opts.pix, item.y);
				context.arc(x, item.y, 3 * opts.pix, 0, 2 * Math.PI, false);
				//end
...
function drawToolTipSplitLine(offsetX, opts, config, context) {
	// 自定义splitLine位置: 阶梯图选中横线中心，非x轴刻度位置
	// if (opts.type === 'area' && opts.extra.area.type === 'step' && opts.extra.area.step === 'middle') {
	// 	if (opts.chartData.calPoints.length > 0) {
	// 		let length = 0;
	// 		let index = 0;
	// 		for (let i = 0; i < opts.chartData.calPoints.length; i++) {
	// 			if (opts.chartData.calPoints[i].length > length) {
	// 				length = opts.chartData.calPoints[i].length;
	// 				index = i;
	// 			}
	// 		}
	// 		const calPoints = opts.chartData.calPoints[index];
	// 		index = opts.tooltip.index;
	// 		if (calPoints.length > 0) {
	// 			if (index + 1 < calPoints.length) {
	// 				offsetX += (calPoints[index + 1].x - calPoints[index].x) / 2;
	// 			} else {
	// 				offsetX -= (calPoints[index].x - calPoints[index - 1].x) / 2;
	// 			}
	// 		}
	// 	}
	// }
	...
	//  splitline开始位置自定义
	var draw = false;
	if (toolTipOption.splitLineCut && Array.isArray(opts.chartData.calPoints)) {
		const index = opts.tooltip.index;
		...
		let cList = {};
		for (let i = 0; i < calPoints.length; i++) {
			if (null !== calPoints[i] && null !== calPoints[i][index]) {
				const y = calPoints[i][index].y;
				const x = calPoints[i][index].x;
				cList[y] = opts.color[i];
				yList.push(y);
				xList.push(x);
			}
		}
		startY = Math.min(...yList);
		if (toolTipOption.splitLineMulticolor) {
			yList = yList.slice().sort((a, b) => a - b);
			yList.push(endY);
			// 不同段不同颜色画线
			...
			for (let i = 0; i < yList.length; i++) {
				...
				if (i + 1 < yList.length) {
					const y2 = yList[i + 1];
					context.setStrokeStyle(c || toolTipOption.gridColor || '#cccccc');
					context.setLineWidth(1 * opts.pix);
					context.beginPath();
					context.moveTo(offsetX, y1);
					context.lineTo(offsetX, y2 - (toolTipOption.dashLength || 6 + 3));
					context.stroke();
					draw = true;
				}
			}
...
function drawMarkLine(opts, config, context) {
	...
	// 自定义标记线的显示隐藏，
	// 1根据图列判断是否显示隐藏标记线
	// 2如果图列是显示的再判断当前标记线的值是否为null
	let newData = null;
	if (markLineOption.filterInvisibleLine) {
		newData = [];
		for (let i = 0; i < opts.series.length; i++) {
			let _series = opts.series[i];
			for (let j = 0; j < markLineOption.data.length; j++) {
				let option = markLineOption.data[j];
				if (option.name === _series.name) {
					if (_series.show && option.value !== null && option.value !== undefined) {
						newData.push(option);
					}
				}
			}
	...
	let points = calMarkLineData(newData || markLineOption.data, opts);

	// 自定义标记线label位置 （最多两条标记线）
	// 1.标记线绘制箭头
	// 2.数据label位于图表内部
	// 3.数据label都放图表右侧，默认放在标记线上方，值大的一定在标记线上方，值小的优先放标记线上方放不下则放下方
	const customLabelPosition = markLineOption.customLabelPosition && points.length <= 2 && points.length > 0;
	const arrowSize = 5 * opts.pix; //箭头大小
	const yAxisZero = opts.height - opts.area[2]; //y轴0值位置，即x轴绘制地方
	let isTwoOverlappingMarkLines = true; //两条标记线是否重叠
	let topMarkLineY = 0; //靠上的标记线坐标y值
	let bottomMarkLineY = 0; //靠下的标记线坐标y值
	if (customLabelPosition) {
		bottomMarkLineY = topMarkLineY = points[0].y;
		let y;
		points.forEach(function(item, index) {
			// 找出靠上方的标记线， y值越小越靠上
			topMarkLineY = Math.min(item.y, topMarkLineY);
			// 找出靠下放的标记线，y值越大越靠下
			bottomMarkLineY = Math.max(item.y, bottomMarkLineY);
			y = y || item.y;
			isTwoOverlappingMarkLines = isTwoOverlappingMarkLines && (y === item.y);
		});
	}
	...
	for (let i = 0; i < points.length; i++) {
		...
		if (item.showLabel) {
			...
			let bgHeight = fontSize + 2 * item.labelPadding * opts.pix;
			let bgStartX = item.labelAlign == 'left' ? opts.area[3] - bgWidth : opts.width - opts.area[1];
			bgStartX += item.labelOffsetX;
			// 自定义标签显示到图表内部
			if (item.labelInsideShow || item.autoDistance) {
				if (item.labelAlign === 'left') {
					bgStartX += bgWidth;
				} else {
					bgStartX -= bgWidth;
				}
			...
			// 自定义标签显示到标记线上
			if (customLabelPosition) {
				const gap = 2 * opts.pix; // 数据标签边框背景距离箭头的间距
				// 默认情况下标记线从正中心穿过数据标签边框和箭头
				const offsetY = bgHeight / 2 + arrowSize / 2 + gap;
				if (isTwoOverlappingMarkLines) { //两条标记线重合或仅有一条标记线
					if (i === 0) {
						bgStartY -= offsetY; //数据标签固定位标记线上方
					} else {
						bgStartY += offsetY; //数据标签固定位标记线下方方
					}
				} else { // 两条不重合的标记线
					if (topMarkLineY === item.y) { // 当前靠上标记线
						bgStartY -= offsetY; //数据标签固定位标记线上方
					} else { // 靠下方的标记线
						// 两条标记线内的可用空间
						const space = item.y - arrowSize / 2 - gap - (topMarkLineY + arrowSize / 2);
						if (space >= bgHeight) {
							bgStartY -= offsetY; //数据标签固定位标记线上方
						} else {
							bgStartY += offsetY; //数据标签固定位标记线下方方
				...
				// yAxisZero：当前x轴绘制位置
				if (bgStartY + bgHeight > yAxisZero) { // 穿过x轴或完全处于x轴走下方
					// bottomMarkLineY topMarkLineY
					// 思路1：靠下方的标签直接重叠到靠上方标签的上面？
					// 重置bgStartY，靠下标记线使用靠上标记线的y位置
					// bgStartY = topMarkLineY - 0.5 * fontSize - item.labelPadding * opts.pix;
					// bgStartY += item.labelOffsetY;
					// bgStartY -= 2 * offsetY;

					// 思路2：放到图表左端？
					// bgStartX = opts.area[3] - bgWidth; //强制放到x轴左侧
					// bgStartX += item.labelOffsetX;
					// if (item.labelInsideShow || item.autoDistance) {
					// 	bgStartX += bgWidth;
					// }
					// 重置bgStartY 将标签放到标记线上方
					// bgStartY = item.y - 0.5 * fontSize - item.labelPadding * opts.pix;
					// bgStartY += item.labelOffsetY;
					// bgStartY -= offsetY; //数据标签固定位标记线上方
				}
			}
			...
			if (item.showLabelBorder) {
				context.setStrokeStyle(item.labelBgColor);
			} else {
				context.setStrokeStyle(hexToRgb('#fff', 0));
			}
			context.setLineWidth(1 * opts.pix);
			context.beginPath();
			if (item.borderRadius) {
				// 假设 context 是你的绘图上下文，bgStartX, bgStartY, bgWidth, bgHeight 是你已知的参数
				const radius = item.borderRadius; // 圆角半径
				drawRRect(context, radius, bgStartX, bgStartY, bgWidth, bgHeight);
			} else {
				context.rect(bgStartX, bgStartY, bgWidth, bgHeight);
			}
			context.closePath();
			context.stroke();
			context.fill();
			...
			// 自定义箭头
			if (item.showLabelArrow) {
				// const arrowSize = 10;
				const arrowStartY = item.y - arrowSize / 2;
				let arrowStartX;
				if (item.labelAlign === 'left') {
					arrowStartX = startX;
				} else {
					arrowStartX = endX - arrowSize;
				}
				drawMarkLineArrow(context, {
					startX: arrowStartX,
					startY: arrowStartY,
					width: arrowSize,
					height: arrowSize,
					arrowSize: arrowSize,
					direction: item.labelAlign,
					padding: 0,
 ...
 * 画圆角矩形
 */
function drawRRect(context, r, x, y, width, height) {
	const radius = r;
	const bgStartX = x;
	const bgStartY = y;
	const bgWidth = width;
	const bgHeight = height;
	context.moveTo(bgStartX + radius, bgStartY); // 从左上角开始，考虑圆角的起点
	// 绘制上边线
	context.lineTo(bgStartX + bgWidth - radius, bgStartY);
	// 圆角右上
	context.arcTo(bgStartX + bgWidth, bgStartY, bgStartX + bgWidth, bgStartY + radius, radius);
	// 绘制右边线
	context.lineTo(bgStartX + bgWidth, bgStartY + bgHeight - radius);

	// 圆角右下
	context.arcTo(bgStartX + bgWidth, bgStartY + bgHeight, bgStartX + bgWidth - radius, bgStartY +
		bgHeight, radius);
	// 绘制下边线
	context.lineTo(bgStartX + radius, bgStartY + bgHeight);
	// 圆角左下
	context.arcTo(bgStartX, bgStartY + bgHeight, bgStartX, bgStartY + bgHeight - radius, radius);
	// 绘制左边线
	context.lineTo(bgStartX, bgStartY + radius);
	// 圆角左上
	context.arcTo(bgStartX, bgStartY, bgStartX + radius, bgStartY, radius);
}

/**
 * 画标记线箭头
 */
function drawMarkLineArrow(context, options) {
	const {
		startX, // 起始点 X 坐标
		startY, // 起始点 Y 坐标
		width, // 矩形宽度
	...
	if (direction === "right") {
		// 箭头指向右侧
		context.moveTo(startX, startY + padding); // 左上角
		context.lineTo(startX + width - arrowSize, startY + padding); // 右上角（箭头开始）
		context.lineTo(startX + width, startY + height / 2); // 箭头尖
		context.lineTo(startX + width - arrowSize, startY + height - padding); // 右下角（箭头结束）
		context.lineTo(startX, startY + height - padding); // 左下角
		context.closePath(); // 回到起点
	} else if (direction === "left") {
		// 箭头指向左侧
		context.moveTo(startX + arrowSize, startY + padding); // 左上角（箭头结束）
		context.lineTo(startX + width, startY + padding); // 右上角
		context.lineTo(startX + width, startY + height - padding); // 右下角
		context.lineTo(startX + arrowSize, startY + height - padding); // 左下角（箭头开始）
		context.lineTo(startX, startY + height / 2); // 箭头尖
		context.closePath(); // 回到起点
	}
...
function drawToolTipSplitArea(offsetX, opts, config, context, eachSpacing) {
	// 柱状图区域阴影修改为基准线
	if (opts.extra.tooltip.splitLineType === "line") {
		drawToolTipSplitLine(offsetX, opts, config, context);
		return;
	}
	// 柱状图区域阴影宽度修最宽改为150
	if (eachSpacing > 150) {
	...
	// 修复混合图阴影超出x轴范围
	// 矩形的起始点的 x 坐标， 原来值
	let areaStartX = offsetX - toolTipOption.activeWidth / 2;
	// 矩形的宽度，沿着 x 轴的尺寸。如果为负数，则矩形向左延伸，原来值
	let areaEndX = toolTipOption.activeWidth;
	if (opts.type === "mix") {
		// 相对于屏幕绘制的x轴坐标
		// const screenAreaStartX = offsetX - toolTipOption.activeWidth / 2 + opts._scrollDistance_;
		const screenAreaStartX = areaStartX + opts._scrollDistance_;
		// X轴起点坐标
		const xAxisStartX = opts.chartData.xAxisData.startX;
		// x轴结束点坐标
		const xAxisEndX = opts.chartData.xAxisData.endX;
		// 如果相对于屏幕绘制的x轴坐标 比 x轴的起点坐标小，则绘制超出了
		if (screenAreaStartX < xAxisStartX) {
			// areaStartX = offsetX - toolTipOption.activeWidth / 2 + (xAxisStartX - screenAreaStartX);
			areaStartX += (xAxisStartX - screenAreaStartX);
			areaEndX -= (xAxisStartX - screenAreaStartX);
		}
		if (screenAreaStartX + areaEndX > xAxisEndX) {
			areaEndX -= (screenAreaStartX + areaEndX - xAxisEndX);
		}
	}
	...
	// 矩形的起始点的 x 坐标。
	// 矩形的起始点的 y 坐标。
	// 矩形的宽度，沿着 x 轴的尺寸。如果为负数，则矩形向左延伸。
	// 矩形的高度，沿着 y 轴的尺寸。如果为负数，则矩形向上延伸。
	context.rect(areaStartX, startY, areaEndX, endY - startY);
	context.closePath();
	context.fill();
...
function drawToolTip(textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
	var toolTipOption = assign({}, {
		...
		shadowOffsetY: 0,
		legendRadius: null,
		textColumnCustom: false,
		textColumnColor: [],
		textColumnGap: [],
		textColumnDelimiter: " ",
		textColumnAlign: null,
	}, opts.extra.tooltip);
	...
	var arrowWidth = toolTipOption.showArrow || toolTipOption.keepArrowSize ? 8 * opts.pix : 0;
	var isOverRightBorder = false;

	// 自定义splitLine位置: 阶梯图选中横线中心，非x轴刻度位置
	// 范围值选择两刻度中心位置
	let offsetX = offset.x; //opts.tooltip.offset.x
	let customX = false;
	if (!opts.tooltip.offset.x) {
		// 自定义梯形图数据存在NaN
		return;
	}
	try {
		const spacing = opts.chartData.eachSpacing / 2; //两刻度点中心位置
		customX = opts.type === 'area' && opts.extra.area.type === 'step' && opts.extra.area.step === 'middle';
		if (customX) {
			const index = opts.tooltip.index;
			const length = opts.chartData.xAxisPoints.length;
			if (index !== length - 1) {
				offsetX += spacing;
			} else {
				offsetX -= spacing;
		...
		console.error("阶梯图选中横线中心，非x轴刻度位置", e);
	}
	// end
	...
	if (opts.type == 'line' || opts.type == 'mount' || opts.type == 'area' || opts.type == 'candle' || opts.type ==
		...
		if (toolTipOption.splitLine == true) {
			// 柱状图区域基准线修改为阴影
			if (opts.extra.tooltip.splitLineType === "area") {
				drawToolTipSplitArea(opts.tooltip.offset.x, opts, config, context, eachSpacing);
			} else {
				// drawToolTipSplitLine(opts.tooltip.offset.x, opts, config, context);
				drawToolTipSplitLine(customX ? offsetX : opts.tooltip.offset.x, opts, config, context);
			}
		}
	}
	offset = assign({
	...
	var textWidth = textList.map(function(item) {
		// 新增逻辑排除分隔符，分隔符不参与计算宽度，使用gap属性替代
		if (toolTipOption.textColumnCustom) {
			return measureText(item.text.split(toolTipOption.textColumnDelimiter).join(""), fontSize, context);
		}
		// 原有计算
	...
	// 文字最大宽度，原来逻辑
	var maxTextWidth = Math.max.apply(null, textWidth);

	// 新增逻辑：第一行左对齐，其他右对齐处理
	let columnsMaxWidth = [];
	let columnsMaxText = [];
	let columnsMaxCount = 0;
	// 水平多组文字显示不同色值，然后在各组中间添加的间距
	let textGap = 0;
	// 自定义各列
	if (toolTipOption.textColumnCustom) {
		textList.forEach(function(item, rowIndex) {
			const strs = item.text.split(toolTipOption.textColumnDelimiter);
			columnsMaxCount = Math.max(columnsMaxCount, strs.length);
			if (!item.ignoreWidthCacl) {
				strs.forEach(function(text, colIndex) {
					const width = measureText(text, fontSize, context);
					if (width > (columnsMaxWidth[colIndex] || 0)) {
						columnsMaxWidth[colIndex] = width;
						columnsMaxText[colIndex] = text;
					}
				});
			}
		});
		...
		// 第一列左对齐其他列右对齐，重新计算宽度
		if (toolTipOption.textColumnAlign === "FirstColumnLeft") {
			maxTextWidth = measureText(columnsMaxText.join(""), fontSize, context);
		}
		// 每列间距
		if (toolTipOption.textColumnGap) {
			for (let i = 0; i < columnsMaxCount - 1; i++) {
				textGap += (toolTipOption.textColumnGap[i] || 10);
			}
			// toolTipOption.textColumnGap.forEach(function(gap, i) {
			// 	if (i < columnsMaxCount - 1) {
			// 		textGap += gap;
			// 	}
			// });
		} else {
			textGap = 10 * (columnsMaxCount - 1);
		}
	}

	// 这里计算toolTip的宽度
	var toolTipWidth = legendWidth + legendMarginRight + 4 * boxPadding + maxTextWidth + textGap;
	...
	if (toolTipHeight + offset.y > opts.height) {
		// -8 在背景带隐形情况下会忽略阴影的大小
		offset.y = opts.height - toolTipHeight - 8;
	}
	...
	// xhr 设置阴影样式
	context.shadowColor = toolTipOption.shadowColor; // 阴影颜色
	context.shadowBlur = toolTipOption.shadowBlur * opts.pix; // 阴影模糊程度
	context.shadowOffsetX = toolTipOption.shadowOffsetX * opts.pix; // 阴影水平偏移量
	context.shadowOffsetY = toolTipOption.shadowOffsetY * opts.pix; // 阴影垂直偏移量
	...
	if (isOverRightBorder) {
		...
		// 检查左侧越界
		if (offset.x - toolTipWidth - arrowWidth - Math.abs(opts._scrollDistance_ || 0) <= 0) {
			offset.x = toolTipWidth + arrowWidth + Math.abs(opts._scrollDistance_ || 0); // 将 Tooltip 固定在左侧
			// offset.x = opts.width + Math.abs(opts._scrollDistance_ || 0) + arrowWidth + (toolTipWidth - opts.width)
		}

		// 画布能放下tooltip时，保证tooltip右侧和画布右侧对齐
		// if ((toolTipWidth + arrowWidth) < opts.width && offset.x > opts.width) {
		// 	offset.x = opts.width;
		// }
			...
			if (item.color !== null) {
				...
				switch (item.legendShape) {
					...
					case 'circle':
						...
						// xhr 修改legend小圆的大小 原值为5 * opts.pix
						const size = opts.extra.tooltip.legendRadius || 5;
						context.arc(startX + 7.5 * opts.pix, startY + 0.5 * legendWidth, size * opts.pix, 0, 2 *
							Math.PI);
						break;
					case 'rect':
	...
	textList.forEach(function(item, index) {
		...
		// tooltip自定义分行显示时，不显示图例时不保留图例的位置
		if (item.delLegend || false) {
			startX = offset.x + arrowWidth + 2 * boxPadding;
		}
		if (isOverRightBorder) {
		...
		if (toolTipOption.textColumnCustom) {
			try {
				const strs = item.text.split(toolTipOption.textColumnDelimiter);
				const colors = item.textColumnColor || toolTipOption.textColumnColor || [];
				const gaps = item.textColumnGap || toolTipOption.textColumnGap || [];
				let offset = 0;
				let previousEndX = 0;
				for (let i = 0; i < strs.length; i++) {
					const str = strs[i];
					const color = colors[i] || '#000000';
					const width = measureText(str, fontSize, context);
					let x;
					let gap = 10;
					if (toolTipOption.textColumnAlign === "FirstColumnLeft") {
						if (i > 0) {
							gap = gaps[i - 1] || 10;
							offset += columnsMaxWidth[i - 1] + gap;
						}
						if (i === 0) {
							x = startX + offset
						} else {
							x = startX + offset + columnsMaxWidth[i] - width;
						}
					} else {
						// 
						if (i > 0) {
							gap = gaps[i - 1] || 10;
							offset += measureText(strs[i - 1], fontSize, context) + gap;
						}
						x = startX + offset
					}
					context.setFillStyle(color);
					context.fillText(str, x, startY);
				}
			} catch (e) {
				console.log("解析错误", e);
				...
				for (let i = 0; i < points.length; i++) {
					...
					if (item !== null && i > leftNum && i < rightNum) {
						...
						if ((columnOption.barBorderRadius && columnOption.barBorderRadius.length === 4) ||
							...
							// 修改圆角大小 默认除以2
							let ratio = columnOption.radiusRatio || 2;
							// 修改圆角大小end
							let minRadius = Math.min(width / ratio, height / ratio);
			...
			case 'stack':
				...
				for (let i = 0; i < points.length; i++) {
					...
					if (item !== null && i > leftNum && i < rightNum) {
						...
						// 给堆叠图添加圆角
						// 判断给哪一根柱子添加圆角
						if (columnOption.barBorderCircle === true) {
							if (stackArcMap.get(i) === null || stackArcMap.get(i) === undefined) {
						...
						if (seriesIndex === stackArcMap.get(i)) {
							const x = startX;
							const y = item.y;
							const width = item.width;
							const h = height;
							...
							// 修改圆角大小 默认除以2
							const ratio = columnOption.radiusRatio || 2;
							// 确保半径不超过矩形宽高的一半
							const r = Math.min(Math.min(width / ratio, h / ratio), width / 2, h / 2);

							// 绘制圆角矩形路径
							context.moveTo(x + r, y);
							context.lineTo(x + width - r, y);
							context.arcTo(x + width, y, x + width, y + r, r); // 右上角
							context.lineTo(x + width, y + h);
							context.lineTo(x, y + h);
							context.lineTo(x, y + r);
							context.arcTo(x, y, x + r, y, r); // 左上角
							context.closePath();
							context.fill();
						} else {
							// 原有代码
...
function drawAreaDataPoints(series, opts, config, context) {
	...
	series.forEach(function(eachSeries, seriesIndex) {
		...
		for (let i = 0; i < splitPointList.length; i++) {
			...
			// 自定义区域图 区域块透明度
			if (eachSeries.opacity !== null && eachSeries.opacity !== undefined) {
				areaOption.opacity = eachSeries.opacity;
			}

			// 绘制区域数
			// 自定义图表 指定某个区域高亮
			const pieces = eachSeries.pieces;
			if (Array.isArray(pieces) && pieces.length > 0) {
				pieces.forEach(function(eachPieces, piecesIndex) {
					context.beginPath();
					context.setFillStyle(hexToRgb(eachPieces.color || eachSeries.color, areaOption
						.opacity));
					if (points.length > 1) {
						let firstPoint = points[eachPieces.gt];
						let lastPoint = points[eachPieces.lt];
						context.moveTo(firstPoint.x, firstPoint.y);
						let startPoint = 0;
						if (areaOption.type === 'step') {
							for (let j = eachPieces.gt; j <= eachPieces.lt; j++) {
								let item = points[j];
								if (startPoint == 0 && item.x > leftSpace) {
									context.moveTo(item.x, item.y);
									startPoint = 1;
			...
			// 自定义功能：画区域值
			if (eachSeries.areaLabel) {
				context.beginPath();
				context.setFontSize(10 * opts.pix);
				context.setFillStyle(eachSeries.color);
				context.setTextAlign('center');
				context.moveTo(points[0].x, points[0].y);
				const firstPoint = points[0];
				const lastPoint = points[points.length - 1];
				const xCenter = firstPoint.x + (lastPoint.x - firstPoint.x) / 2;
				const yCenter = firstPoint.y - 12;
				const labels = eachSeries.areaLabel.split('\n'); // 按换行符拆分为多
				const lineHeight = 10 * opts.pix;
				let drawLabel = true;
				// 排查文字可能显示不下的情况
				for (let i = 0; i < labels.length; i++) {
					const textWidth = measureText(labels[i], 10 * opts.pix, context);
					const areaWidth = lastPoint.x - firstPoint.x;
				...
				if (drawLabel) {
					labels.forEach((text, index) => {
						context.fillText(text, xCenter, yCenter - index * lineHeight);
					});
				}
				// context.fillText(String(eachSeries.areaValue), xCenter, yCenter - 2);
				// context.fillText(eachSeries.areaLabel, xCenter, yCenter - lineHeight);
				context.closePath();
				context.stroke();
			}
		}
		...
		// 删除激活点
		const indexs = eachSeries.delActivePointIndex;
...
function drawMixDataPoints(series, opts, config, context, _process) {
	let process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
	let xAxisData = opts.chartData.xAxisData,
		xAxisPoints = xAxisData.xAxisPoints,
		eachSpacing = xAxisData.eachSpacing;
	...
	series.forEach(function(eachSeries, seriesIndex) {
		...
		var data = eachSeries.data;
		// 将getDataPoints修改为getColumnDataPoints
		var points = getColumnDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts,
			config, zeroPoints, process);
		// var points = getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, process);
		calPoints.push(points);
		...
		if (eachSeries.type == 'area') {
			...
			for (let i = 0; i < splitPointList.length; i++) {
				...
				if (areaOption.gradient) {
					...
					context.setFillStyle(hexToRgb(eachSeries.color, eachSeries.bgAlpha ? 1 : areaOption
						.opacity)); //背景透明度修改
				}
				context.setLineWidth(2 * opts.pix);
				if (points.length > 1) {
		...
		if (eachSeries.type == 'line') {
			lineOption.width = eachSeries.width; //添加 混合图 自定义线条宽度
			var splitPointList = splitPoints(points, eachSeries);
			splitPointList.forEach(function(points, index) {
				if (eachSeries.lineType == 'dash') {
					let dashLength = eachSeries.dashLength ? eachSeries.dashLength : 8;
		...
		if (eachSeries.addPoint == true && eachSeries.type !== 'column') {
			// 图表穿模bug
			let fixedBugPoints = null;
			const indexs = []; //当前在X轴范围内的点
			try {
				if (opts.chartData.calPoints) {
					const obj = {};
					for (let i = 0; i < opts.series.length; i++) {
						const key = opts.series[i].name;
						const value = opts.chartData.calPoints[i];
						obj[key] = value;
					}
					if (!opts.chartData.fixedBug4DrawPointShape) {
						// 缓存calPoints数据
						opts.chartData.fixedBug4DrawPointShape = obj;
					}
				}
				// 点击图例时calPoints的数据会变化，fixedBug4DrawPointShape为calPoints数据集的原始数据
				if (opts.chartData.fixedBug4DrawPointShape) {
					const arr = opts.chartData.fixedBug4DrawPointShape[eachSeries.name];
					...
					if (arr) {
						for (let i = 0; i < arr.length; i++) {
							if (arr[i]) {
								const x = arr[i].x;
								const dis = x - Math.abs(opts._scrollDistance_ || 0);
								if (dis >= startX && dis <= endX) { //X轴范围内
									indexs.push(i);
									if (indexs.length === opts.xAxis.itemCount) {
										break;
									}
								}
							}
				...
				// 初始化时 indexs:[], _scrollDistance_:undefined 正常
				// 点击图列图表从有到无 inde:[],  _scrollDistance_:0 异常
				// 滚动后如果数据点个数小于itemCount  异常
				if (indexs.length > 0 && indexs.length <= opts.xAxis.itemCount) {
					// 有数据点，可能混动且点的数量不够itemCount时
					const length = Math.min(indexs.length, opts.xAxis.itemCount);
					const startIndex = indexs[0];
					const endIndex = indexs[length - 1] + 1;
					fixedBugPoints = points.slice(startIndex, endIndex);
				} else if (opts._scrollDistance_ === undefined) {
					// 未滚动 首次加载图表
					fixedBugPoints = points.slice(0, opts.xAxis.itemCount);
				}
			} catch (e) {
				console.log("图表穿模-重新计算绘制点异常：", e);
			}

			drawPointShape(fixedBugPoints || points, eachSeries.color, eachSeries.pointShape, context, opts);
		}
	});
	if (opts.dataLabel !== false && process === 1) {
		var columnIndex = 0;
...
function drawXAxis(categories, opts, config, context) {
	...
	//绘制滚动条
	if (opts.enableScroll && opts.xAxis.scrollShow) {
		// -3 * otps.pix 修复在legend在顶部时，滚动条显示不完整bug
		var scrollY = opts.height - opts.area[2] + config.xAxisHeight - 3 * opts.pix;
	...
	if (opts.xAxis.disabled !== true) {
		...
		// 120 / 3  40
		// 15:15   15:55  16:35   17:15
		// 0    8    16   24   25-1/3
		// let parts = opts.xAxis.labelCount - 1;
		// let ratio = Math.ceil((categories.length - 1) / parts);

		let ratio = Math.ceil(categories.length / maxXAxisListLength);

		// 自定义x轴间隔
		if (opts.xAxis.interval) {
			ratio = opts.xAxis.interval;
		}
...
function drawYAxisGrid(categories, opts, config, context) {
	...
	// 对称Y轴
	// 0刻度网格线自定义，解决正负轴时y===0刻度问题，不再需要标记线替代0值网格线
	if (opts.yAxis.enableZeroTickGridCustom) {
		const yAxisTickSum = opts.chartData.yAxisData.ranges[0].reduce((acc, cur) => acc + cur, 0);
		if (yAxisTickSum === 0) { // Y轴对称--正负Y轴
			const yZeroTickIndex = Math.floor(points.length / 2) - startY;
			const item = points[yZeroTickIndex];
			context.save();
			if (opts._scrollDistance_ && opts._scrollDistance_ !== 0) {
				context.translate(opts._scrollDistance_, 0);
			}
			if ((opts.yAxis.zeroTickGridType ?? opts.yAxis.gridType) == 'dash') {
				context.setLineDash([opts.yAxis.dashLength * opts.pix, opts.yAxis.dashLength * opts.pix]);
			}
			context.setStrokeStyle(opts.yAxis.zeroTickGridColor ?? opts.yAxis.gridColor);
			// 设置Y轴grid线
			context.setLineWidth(0.5 * opts.pix);
			context.beginPath();
			context.moveTo(startX, item);
		...
		for (let i = 0; i < opts.yAxis.data.length; i++) {
			...
			if (yData.disabled !== true) {
				...
				if (opts.yAxis.showTitle) {
					...
					// let title2;
					// try {
					// 	title2 = opts.chartData.legendData.points[0][0].yAxisTitle
					// } catch (e) {
					...
					// 新增剔除某些标题
					let excludeTitles = opts.yAxis.excludeTitles;
					if (Array.isArray(excludeTitles) && excludeTitles.includes(title)) {
						title = "";
					}
					...
					if (yAxisWidth.position == 'left') {
						if (yData.titleAndTickAlignment) { // 标题和刻度值使用同一种对齐方式
							let gapwidth = 4 * opts.pix;
							let tmpstrat = 0;
							//画刻度线修改间隔
							if (yData.calibration == true) {
							...
							context.fillText(title, tmpstrat + (yData.titleOffsetX || 0), opts.area[0] - (10 - (yData
								.titleOffsetY || 0)) * opts.pix);
						} else {
		...
		for (let i = 0; i < itemList.length; i++) {
			...
			switch (item.legendShape) {
				...
				case 'circle':
					...
					// xhr 修改小圆大小 原值5
					const size = item.radius || 5;
					context.arc(startX + 7.5 * opts.pix, startY + 0.5 * lineHeight, size * opts.pix, 0, 2 * Math
						.PI);
					break;
				case 'rect':
					...
					break;
				case 'rectA':
					// startY + 0.5 * lineHeight 中心点
					let x = startX + 6;
					// 中心点向上移动 2 * opts.pix；所以高度：2 * 2 * opts.pix;
					let y = startY + 0.5 * lineHeight - 2 * opts.pix;
					let width = 8 * opts.pix;
					let hegiht = 4 * opts.pix;
					context.moveTo(x, y);
					// item.legendShape
					// fillRect(x, y, width, height)
					context.fillRect(x, y, width, hegiht);
					break;
				case 'rectB': //圆角矩形
					// startY + 0.5 * lineHeight 中心点
					x = startX + 8 * opts.pix;
					// 中心点向上移动 2 * opts.pix；所以高度：2 * 2 * opts.pix;
					y = startY + 0.5 * lineHeight - 2 * opts.pix;
					width = 8 * opts.pix;
					hegiht = 4 * opts.pix;
					context.moveTo(x, y);
					// item.legendShape
					// fillRect(x, y, width, height)
					// context.fillRect(x, y, width, hegiht);
					drawRRect(context, 2, x, y, width, hegiht);
					break;
				case 'square':
			...
			// const x1 = (item.radius || 0) * 4; //缩小图表与文案的间距
			// context.fillText(legendText, startX - x1, startY + fontTrans);
			context.closePath();
			context.stroke();
			if (opts.legend.position == 'top' || opts.legend.position == 'bottom') {
				...
				while (true) {
					...
					if (isSpin) {
						...
						areav = [opts.width - (opts.width / 2 - opts.height / 2) - (-x + tHeight + 5 + opts.height /
								2) - 5, (opts.height / 2 - opts.width / 2) + (y - 5 - tWidth + opts.width / 2) - 5,
							opts
...
function drawCharts(type, opts, config, context) {
	...
	// 自定义左右对齐，图表从某个X轴刻度显示
	if (opts.enableScroll && opts._series_ && opts._series_.length > 0) {
		const scrollAlign = opts._series_[0].scrollAlign;
		const srcollIndex = opts._series_[0].srcollIndex;
		if (scrollAlign) {
			opts.xAxis.scrollAlign = scrollAlign;
		}
		// 柱状图Column
		if (srcollIndex > 0) {
			opts._series_[0].srcollIndex = null;
			let xAxisPoints = opts.chartData.xAxisData.xAxisPoints;
			let eachSpacing = opts.chartData.xAxisData.eachSpacing;
			// let distance = xAxisPoints[srcollIndex] + Math.round(eachSpacing / 2);
			let distance = eachSpacing * srcollIndex;
			let offsetLeft = -distance;
			...
			_this.scrollOption.position = offsetLeft;
			opts._scrollDistance_ = offsetLeft;
			opts.xAxis.scrollPosition = offsetLeft;
		}
	}
	...
	if (this.opts.type === 'line' || this.opts.type === 'area' || this.opts.type === 'column' || this.opts.type ===
		...
		if (index > -1 || index.length > 0) {
			...
			if (seriesData.length !== 0) {
				...
				// 获取splitline的位置
				if (option.callback) {
					option.callback(option.offset !== undefined ? option.offset : offset);
				}
